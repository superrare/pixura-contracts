...
contract PixuraNFT is ERC721Full, IERC721Creator, Ownable, Whitelist, Operated {
    ...
    ) ERC721Full(_name, _symbol) Operated(_operator, _operationCost) {}
        ...
        onlyTokenOwner(_tokenId)
    {
        require(
            _exists(_tokenId),
            "token must exist to update the owner metadata"
        );
    ...
    */
     ...
     * @param _uri string metadata uri associated with the token
     */
    function createToken(string _uri, address _creator)
        private
        returns (uint256)
    {
