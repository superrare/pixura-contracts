...
module Contracts.V5.TestExpensiveWallet where
...
type ClaimMoneyFn = Tagged (SProxy "claimMoney(address)") (Tuple1 (Tagged (SProxy "_escrowAddress") Address))

claimMoney :: TransactionOptions NoPay -> { _escrowAddress :: Address } -> Web3 HexString
claimMoney x0 r = uncurryFields  r $ claimMoney' x0
   where
    claimMoney' :: TransactionOptions NoPay -> (Tagged (SProxy "_escrowAddress") Address) -> Web3 HexString
    claimMoney' y0 y1 = sendTx y0 ((tagged $ Tuple1 y1) :: ClaimMoneyFn)
